...
export function parseQuestions(markdownContent) {
  const questions = [];
  const questionBlocks = markdownContent.split('### ').slice(1);
  ...
  questionBlocks.forEach(block => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (line.startsWith('- [')) {
        ...
        if (isCorrect) {
          correctAnswers.push(currentOptionIndex);
        }
        currentOptionIndex++;
      }
    }
...
export const pspoQuestions = parseQuestions(markdownContent);